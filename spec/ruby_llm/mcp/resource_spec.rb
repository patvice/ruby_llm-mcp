...
RSpec.describe RubyLLM::MCP::Resource do
  ...
  # Resource subscription tests - only run on adapters that support subscriptions
  each_client_supporting(:subscriptions) do
  ...
  # Refresh functionality - only supported by adapters with resource_refresh support
  each_client_supporting(:list_changed_notifications) do
