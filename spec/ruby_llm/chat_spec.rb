...
RSpec.describe RubyLLM::Chat do
  ...
  # Tests for features supported by all adapters (tools, resources)
  each_client do
    COMPLEX_FUNCTION_MODELS.each do |model_config|
  ...
  # Tests for resource templates (only supported by native/ruby_llm adapter)
  each_client_supporting(:resource_templates) do
    COMPLEX_FUNCTION_MODELS.each do |model_config|
      context "with #{model_config[:provider]}/#{model_config[:model]}" do
  ...
  # Tests for prompts (only supported by native/ruby_llm adapter)
  each_client_supporting(:prompts) do
    COMPLEX_FUNCTION_MODELS.each do |model_config|
      context "with #{model_config[:provider]}/#{model_config[:model]}" do
